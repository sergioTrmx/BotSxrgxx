 /*
* 𝗦𝘅𝗿𝗴𝘅𝘅-𝗕𝗼𝘁 es una creación de 𝗦𝘅𝗿𝗴𝘅𝘅
* 𝗦𝘅𝗿𝗴𝘅𝘅-𝗕𝗼𝘁 no tiene ningun fin de lucro
* 𝗦𝘅𝗿𝗴𝘅𝘅 se reserva todos los derechos de autor
* © 2021 𝗦𝘅𝗿𝗴𝘅𝘅, INC.
...
/******FIN DE ARCHIVOS ANTILINK POR 𝗦𝘅𝗿𝗴𝘅𝘅******/
...
function addMetadata(packname, author) {	
	if (!packname) packname = '𝗦𝘅𝗿𝗴𝘅𝘅'; if (!author) author = '@Sxrgxx.02';	
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa 𝗦𝘅𝗿𝗴𝘅𝘅-𝗕𝗼𝘁',
...
if (budy.includes("https://wa.me/")){
		if (!isGroup) return
		if (!isAntiWa) return
                if (isGroupAdmins) return reply('uy pana te salvaste por ser admin :)')
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DE WHATSAPP DETECTADO 📢* ${sender.split("@")[0]} Vamos a acabar contigo 😈`)
		setTimeout( () => {
...
/******JUEGOS 𝗦𝘅𝗿𝗴𝘅𝘅 LA PUTA MADRE NO TE OLVIDES******/
...
if (!isGroup) return reply(mess.only.group)
d = []
teks = 'Top 5 de los mas guapos del grupo🙈\n\n'
for(i = 0; i < 5; i++) {
r = Math.floor(Math.random() * groupMetadata.participants.length + 0)
teks += `➔ @${groupMembers[r].jid.split('@')[0]}\n`
d.push(groupMembers[r].jid)
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			//ANTILINKS DE REDES SOCIALES FLACO ACEPTALO SOLO LO ESTAS EDITANDO REALMENTE 𝗦𝘅𝗿𝗴𝘅𝘅 TE HIZO TODO ESTO	
				...
				//FIN DE ANTILINK HECHO POR 𝗦𝘅𝗿𝗴𝘅𝘅
	...
	//SERVICIO DE MUSICA Y VIDEO 			
		...
		if (!isUser) return reply(mess.only.daftarB)
	        reply(mess.only.musica2)
                play = body.slice(5)
                anu = await fetchJson(`https://api.zeks.me/api/ytplaymp3?q=${play}&apikey=𝗦𝘅𝗿𝗴𝘅𝘅50`)
                if (anu.error) return reply(anu.error)
                infomp3 = `*⌜Cancion Encontrada ✅⌟*\n◉ *Título:* ${anu.result.title}\n◉ *Fuente:* ${anu.result.source}\n◉ *Tamaño:* ${anu.result.size}\n\n*ESPERE ENVIANDO SU ARCHIVO MP3 ⚠*\n\n_*Servicio proveido por 𝗦𝘅𝗿𝗴𝘅𝘅*_`
                buffer = await getBuffer(anu.result.thumbnail)
                lagu = await getBuffer(anu.result.url_audio)
